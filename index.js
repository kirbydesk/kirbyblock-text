...
(function() {
  ...
  const _sfc_main$4 = {
    ...
    computed: {
      parsedData() {
        var _a2;
        const val = ((_a2 = this.content) == null ? void 0 : _a2.text) || this.value;
        if (!val) return { text: "", align: "left" };
        try {
          return typeof val === "string" ? JSON.parse(val) : val;
        } catch (e) {
  ...
  const pwToggleGridTab = {
    ...
    methods: {
      claimActiveDrawerForGrid() {
        const closedDrawers = document.querySelectorAll(DRAWER_CLOSED_SELECTOR$2);
      ...
      setGridDrawerClass() {
        const drawer = document.querySelector(`${DRAWER_SELECTOR$2}[data-block-id="${this.id}"]`);
        if (drawer) {
          const isOff = this.content.togglegrid === false || this.content.togglegrid === "false";
  ...
  const pwToggleSpacingTab = {
    ...
    methods: {
      claimActiveDrawerForSpacing() {
        const closedDrawers = document.querySelectorAll(DRAWER_CLOSED_SELECTOR$1);
        closedDrawers.forEach((drawer) => drawer.removeAttribute("data-block-id"));
      ...
      setSpacingDrawerClass() {
        const drawer = document.querySelector(`${DRAWER_SELECTOR$1}[data-block-id="${this.id}"]`);
        if (drawer) {
          const isOff = this.content.togglespacing === false || this.content.togglespacing === "false";
